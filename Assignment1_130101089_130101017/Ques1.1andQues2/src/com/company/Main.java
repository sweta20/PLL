    ...
    public static class operate implements Runnable{
        ...
        * This function takes the raw data from all sensors and perform operation add, average and multiply*/
            ...
            public void run() {
                while(true) {
                    ...
                    for (int i = 0; i < 10; i++) {
                        ...
                        int ele = Main.queues[i].remove();
                        sum += ele;
                        mul *= ele;
                        myIntArray[i] = ele;
                    }
                    //Sort the numbers and print the output
                    synchronized (this){
                        DivideTask d = new DivideTask(myIntArray);
                        int[] resultArray = d.compute();
        ...
        public static void mainFunc() {
            //A list of threads is created to handle task of each sensor concurrently
            ...
            //This thread computes the sum, average, multiplied values and compares them
            Runnable computeTask = new operate();
            Thread computeWorker = new Thread(computeTask);
            computeWorker.start();
        }
